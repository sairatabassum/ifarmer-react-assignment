...
export const ProductCard = ({ product, onDelete }: { product: Product; onDelete: (product: Product) => void; }) => (
    <div className="bg-white rounded-xl shadow-md overflow-hidden transition-transform duration-200 hover:scale-105 cursor-pointer">
        <Link href={`/assignment-2/details/${product.id}`} className="block">
        ...
        <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 px-4">
            <Link
                href={`/assignment-2/details/${product.id}`}
